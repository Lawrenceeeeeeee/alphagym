# AlphaGYM — Quant Research Toolbox 开发规范

## 产品定位与对外文案

- AlphaGYM — Quant Research Toolbox 定位为可扩展的多市场、多资产量化研究工具箱；中国 A 股是
  当前首个落地研究市场，而不是产品的长期边界。未来可扩展至期货、期权、ETF、数字资产等市场
  与资产类别，设计新模块和公共抽象时应保留相应扩展空间。
- Release notes 只简洁记录该版本已经交付的功能、修复和可验证变化，不写产品愿景、未来路线或
  尚未交付的能力。产品定义、发展方向和路线规划统一维护在 README 的 To-do list 中。
- 对外描述产品时使用“Quant Research Toolbox / 量化研究工具箱”；ClickHouse 是当前存储技术，
  不作为产品名称或产品定位。
...
- 复用逻辑放 `src/alphagym/`；定义与配置放 `research/`、`config/`；大数据与产物不得提交。
...
- 报告创建双入口：CLI `alphagym report create --spec x.yaml [--run]`（agent 首选）与 Web
  `/reports/new` 表单；后台执行、`alphagym report wait` 轮询、进度写入 `report` 表。
...
- 三条标准工作流：
  1. 体检：`alphagym status --root <root> --json`（数据库表行数/数据版本/进行中任务）；
  2. 发现：`alphagym factor list --json`、`alphagym factor show <id> --json`、
     `alphagym run show --run-id <id> --json`、`alphagym report list --json`；
  3. 研究：写 report spec YAML → `alphagym report create --spec x.yaml --run --json`
...
- 复权因子在 `{datadir}/DividData` 的 LevelDB（`.ldb`）中，独立于日线。key 为
  ...
  `bytes 72..79`（float64）是单次除权的后复权因子。解析器在 `src/alphagym/qmt_dividend.py`
...
- `adjust_factor` 是累计后复权因子，仅在除权日有记录（该日及之后生效），除权前为 1.0；使用时
  `ffill().fillna(1.0)`。
- 入库命令：`python -m alphagym.cli equity-data import-qmt --datadir <QMT datadir> --root <数据根>`，
