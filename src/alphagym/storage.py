...
class ClickHouseConfig:
    ...
    def load(cls, root: Path) -> ClickHouseConfig:
        ...
        if "password" in values:
            raise StorageError("Use ALPHAGYM_CLICKHOUSE_PASSWORD, not a password in storage.yaml")
        for name in ("host", "port", "username", "password", "database", "secure", "workspace"):
            value = os.environ.get("ALPHAGYM_CLICKHOUSE_" + name.upper())
            if value is not None:
                values[name] = value
        # Keep compatibility with the existing Docker deployment variable.
        if "ALPHAGYM_CLICKHOUSE_USERNAME" not in os.environ and os.environ.get("ALPHAGYM_CLICKHOUSE_USER"):
            values["username"] = os.environ["ALPHAGYM_CLICKHOUSE_USER"]
        if "port" in values:
            values["port"] = int(values["port"])
        if "secure" in values:
            values["secure"] = str(values["secure"]).lower() in {"true", "1", "yes"}
        if not re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", values.get("database", "alphagym")):
...
class WriteBatch:
    ...
    def frame(self, path, frame, *, mode="replace", keys=None, index=False, schema=None):
        ...
        if path.startswith("equity/") and path.endswith(".parquet"):
            from alphagym.equity_data import DATE_COLUMNS, SCHEMAS
