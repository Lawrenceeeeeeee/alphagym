...
from alphagym.combine import combine_scores, factor_weights
from alphagym.ml_composite import frozen_composite, load_pit_context, neutralize_wide
from alphagym.report_engine import build_cross_section
from alphagym.report_spec import parse_spec
from alphagym.research import zscore
from alphagym.workflows._support import finish, invoke, progress, report_record, validate_config
...
def _scores_by_month(
    root: Path, report_row: dict, method: str,
) -> dict[pd.Timestamp, pd.Series]:
    rep = Path(report_row["path"])
    spec = parse_spec(json.loads(report_row["spec_json"]))
    manifest = json.loads(storage_io.read_text(rep / "manifest.json", encoding="utf-8"))
    combo = json.loads(storage_io.read_text(rep / "combo.json", encoding="utf-8"))
...
def run(args: Config) -> dict:
    ...
    root = Path(args.root)
    report_row = report_record(root, args.report_id)
    ...
    member_symbols = None
    if index_code != "ALL_A":
        members = storage_io.read_frame(root / "equity" / "index_members.parquet")
    ...
    if not nav_rows:
        raise ValueError("no rebalances executed")
    ...
    progress(f"[backtest] {index_code} {args.method} top-{args.top_n} | "
          f"{'无成本' if args.zero_cost else f'滑点{args.slippage_bps}bps+佣金3bp+印花税+过户费'}")
    progress(perf.round(4).to_string())
    ...
    progress(f"成本: 手续费 {costs:,.0f} + 滑点 {slippage_cost:,.0f} = {(costs + slippage_cost):,.0f} 元 "
          f"(约占年均净值 {100 * (costs + slippage_cost) / avg_equity / (len(nav) / 12):.2f}%/年)")
    if args.output:
        out = Path(args.output)
        out.mkdir(parents=True, exist_ok=True)
        storage_io.write_csv(nav, out / "nav.csv", index=False)
        storage_io.write_csv(trades, out / "trades.csv", index=False)
        storage_io.write_csv(perf, out / "performance.csv")
        progress(f"saved -> {out}")
    result = finish(output=args.output)
    result["performance"] = perf.reset_index().to_dict("records")
    return result
