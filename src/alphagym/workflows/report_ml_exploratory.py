...
from alphagym.research import newey_west_t
from alphagym.workflows._support import finish, invoke, validate_config
...
def common_test_results(output: Path, summary: pd.DataFrame) -> tuple[pd.DataFrame, str]:
    ...
    for row in summary[summary.period.eq("test")].itertuples():
        frame = storage_io.read_frame(output / row.frequency / row.method / f"{row.scenario}.parquet")
    ...
    storage_io.write_csv(result, output / "common_test_summary.csv", index=False, encoding="utf-8-sig")
    return result, f"{start.date()} 至 {end.date()}（共同开盘估值窗口）"


def write_report(output: Path) -> None:
    summary = storage_io.read_csv(output / "summary.csv")
    expected = 3 * 15 * 4 * 4
    ...
    for frequency, method in selected.items():
        frame = storage_io.read_frame(output / frequency / method / "base_5bps.parquet")
    ...
    storage_io.write_bytes(static / "echarts.min.js", storage_io.read_bytes(files("alphagym").joinpath("static/echarts.min.js")))
...
<script src="static/echarts.min.js"></script><script>const options={safe_chart};const charts=options.map((o,i)=>{{const c=echarts.init(document.getElementById('chart'+i));c.setOption(o);return c;}});addEventListener('resize',()=>charts.forEach(c=>c.resize()));</script></html>'''
    storage_io.write_text(output / "report.html", document, encoding="utf-8")
    storage_io.write_text(output / "validation_selection.json", json.dumps(selected, indent=2), encoding="utf-8")
...
def run(args: Config) -> dict:
    ...
    return finish(output=args.output / "report.html")
...
def main(argv: list[str] | None = None) -> int:
    ...
    return invoke(run, Config, parser.parse_args(argv))
