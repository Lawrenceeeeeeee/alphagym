...
def resolve_root(root: str | Path | None = None) -> Path:
    value = root if root is not None else os.environ.get("ALPHAGYM_DATA_ROOT")
    if value is None or not str(value).strip():
        raise DataContractError("--root or ALPHAGYM_DATA_ROOT is required")
    from alphagym.storage_io import register_root
