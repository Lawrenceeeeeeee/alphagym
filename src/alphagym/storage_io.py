...
_snapshots = ContextVar("alphagym_data_snapshots", default=MappingProxyType({}))
...
def locate(path) -> tuple[Path, str]:
    ...
    env = os.environ.get("ALPHAGYM_DATA_ROOT")
    if env:
        register_root(env)
...
def store_for(root, *, initialize=False):
    ...
    from alphagym.storage import ClickHouseConfig
...
def exists(path):
    ...
    if path.name == "catalog.sqlite":
        from alphagym.catalog_connection import catalog_exists
