...
def _covered(store: FactorStore, factor_id: str, index_code: str, start: str, end: str) -> bool:
    ...
    if panel_row is None or not Path(str(panel_row["path"])).is_file():
        return False
    panel_max = pd.to_datetime(
        pd.read_parquet(Path(str(panel_row["path"])), columns=["signal_date"])["signal_date"]
    ).max()
...
def main() -> int:
    ...
    payload = yaml.safe_load(Path(args.pool).read_text(encoding="utf-8"))
    ...
    for index_code in indices:
        with FactorStore.from_root(Path(args.root)) as store:
            ...
            if not missing:
                print(f"[backfill] {index_code}: all {len(pool)} factors covered, skip", flush=True)
                continue
            print(
            ...
            for offset in range(0, len(missing), args.batch_size):
                ...
                try:
                    ...
                    print(f"[backfill] {index_code}: OK -> {output}", flush=True)
                except Exception as error:  # noqa: BLE001 - report and continue
                    results.append({"index_code": index_code, "run_id": run_id,
                                    "factors": len(batch), "status": "failed",
                                    "error": str(error)})
                    print(f"[backfill] {index_code}: FAILED: {error}", flush=True)
    if args.json:
