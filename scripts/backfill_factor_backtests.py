...
from mlquant.report_spec import FactorSelection, MetricFilter, ReportSpec, resolve_factors
...
def _build_family_reports(store: FactorStore, mode: str, top_n: int) -> None:
    ...
    for family in families:
        ...
        if len(resolved) < 2:
            print(f"[reports] skip family {family}: only {len(resolved)} factors", flush=True)
            continue
        print(
        ...
        try:
            ...
            print(f"[reports] done {family} -> {output}", flush=True)
        except Exception as error:  # noqa: BLE001 - one bad family must not stop the rest
            print(f"[reports] FAILED {family}: {error}", flush=True)
...
def main() -> int:
    ...
    with FactorStore.from_root(root) as store:
        if args.factors:
            ...
            todo, _blocked = _collect_todo(
                store, explicit, mode, args.market_only, args.start_date, args.end_date
            )
            if not todo:
                print(f"batch skipped: all {len(explicit)} factors already succeeded", flush=True)
                return 0
            run_id, message = _run_batch(store, todo, mode, args.start_date, args.end_date)
            print(f"[batch] run={run_id} factors={len(todo)} {message}", flush=True)
            return 0
        ...
        if not todo:
            print("nothing to do: all runnable factors already backtested", flush=True)
        ...
        if args.workers <= 1:
            for number, batch in enumerate(batches, start=1):
                started = time.time()
                print(f"[batch {number}/{len(batches)}] factors={len(batch)} first={batch[0]}", flush=True)
                run_id, message = _run_batch(
                    store, batch, mode, args.start_date, args.end_date
                )
                print(f"[batch {number}] {time.time() - started:.0f}s run={run_id} {message}", flush=True)
            return 0
        ...
        while next_batch < len(batches) or running:
            while next_batch < len(batches) and len(running) < args.workers:
                ...
                print(f"[spawn {next_batch + 1}/{len(batches)}] factors={len(batch)} first={batch[0]}", flush=True)
            ...
            for label, batch, process in finished:
                code = process.wait()
                results.append((len(batch), f"{label} exit={code}"))
