...
def _feature_files(
    ...
    signature = hashlib.sha256(json.dumps({
        ...
        "feature_code": hashlib.sha256(Path(sys.modules[market_features.__module__].__file__).read_bytes()).hexdigest(),
    ...
    for year in range(start.year, end.year + 1):
        ...
        print(f"cached {frequency} {year}: {len(frame):,} rows", flush=True)
...
def _fit_model(
    ...
    print(f"fit {model}: {len(x_train):,} rows", flush=True)
...
def main() -> int:
    ...
    if spec.universe.index_code != "ALL_A":
        raise SystemExit("high-frequency experiment currently requires the ALL_A report")
        ...
        def evaluate(
            ...
            *,
            ...
            for scenario in DEFAULT_COST_SCENARIOS:
                ...
                print(f"finished {frequency_name} {method} {scenario.name}", flush=True)
    ...
    pd.concat(return_blocks, ignore_index=True).to_parquet(output / "returns.parquet", index=False)
    metadata = {
        "report_id": args.report_id,
