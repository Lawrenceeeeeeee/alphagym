...
首次使用请先阅读 [从零开始使用 MLQuant](getting-started.md)，完成 Python 环境、数据根
...
mlquant factor sync --root /path/to/local/data --json
mlquant factor show MOMENTUM_60D --root /path/to/local/data --json
...
mlquant report list --root /path/to/local/data --json
mlquant report export --root /path/to/local/data --report-id <returned-id> --output /path/to/export --json
...
业务实现位于 `mlquant.workflows`，`scripts/` 中的同名入口只是兼容包装。
不需要复制源码脚本，不默认读取私有 `config/ml_pool.yaml`。

```python
from pathlib import Path
from mlquant.workflows.run_ml_exploration import Config, run
...
业务函数通过 `mlquant.workflows` logger 记录进度，不写标准输出、不退出宿主进程。

Agent 可用 `mlquant workflow list --json` 发现九条流程，用
`mlquant workflow describe <name> --json` 读取参数、类型、默认值与必填标志，再将配置
写成 YAML，通过 `mlquant workflow run <name> --config /path/to/workflow.yaml --json` 执行。
Python 对应 `mlquant.workflows.catalog.run_workflow(name, config)`；只允许注册的工作流名称。
